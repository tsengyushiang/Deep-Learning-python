...
class MultinomialLogisticRegression(object):
    ...
    def initialize_weights(self, input_dim, num_classes, std_dev=1e-2):
        ...
        #############################################################################
        self.params['W'] = (np.random.randn(input_dim*num_classes)*std_dev).reshape(input_dim,num_classes)
        self.params['b'] = np.repeat(0.0,num_classes)
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################        
    ...
    def softmax(self,x):
        ...
        def softmax(x,c):
            ...
            return softmax_x 
        probs = np.array([softmax(row,-np.max(row)) for row in x])
        #############################################################################
        #                              END OF YOUR CODE                             #
        ############################################################################# 
    ...
    def predict(self, X):
        ...
        predictions = self.softmax(X@W+b)
